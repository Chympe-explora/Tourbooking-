...
(function(){
  ...
  function saveOverrides(){
    ...
    if(window.KC_FIREBASE_URL){
      showSaveStatus('Saving…', false);
      fetch(window.KC_FIREBASE_URL+'/'+STORAGE_KEY+'.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(overrides)})
        .then(function(r){
          if(!r.ok) throw new Error('HTTP '+r.status+' '+r.statusText);
          showSaveStatus(localErr ? ('Saved to cloud — visible to every visitor. (Device cache skipped: '+localErr+')') : 'Saved — visible to every visitor.', false);
        })
        .catch(function(err){
          showSaveStatus('⚠️ Not visible to other visitors yet — cloud sync failed: '+(err && err.message ? err.message : err), true);
        });
    } else if(localErr){
      showSaveStatus('❌ Save failed: '+localErr, true);
    } else {
      showSaveStatus('Saved on this device only. Set KC_FIREBASE_URL to publish for every visitor.', true);
    }
  }
  ...
  function init(){
    ...
    bindLogoTap();
    if(window.KC_FIREBASE_URL){
      fetch(window.KC_FIREBASE_URL+'/'+STORAGE_KEY+'.json').then(function(r){return r.json()}).then(function(remote){
        if(remote){
          overrides = sanitizeOverrides(remote);
          localStorage.setItem(STORAGE_KEY, JSON.stringify(overrides));
        }
        waitForRootThenStart(20);
      }).catch(function(){ waitForRootThenStart(20); });
    } else {
      waitForRootThenStart(20);
    }
  }
